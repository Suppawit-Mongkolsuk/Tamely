...
export const updateWorkspaceIcon = async ( // อัพโหลดหรือเปลี่ยนไอคอนของ workspace
  workspaceId: string,
  userId: string,
  fileBuffer: Buffer,
  mimeType: string,
  ...
  const allowed = await hasPermission(workspaceId, userId, PERMISSIONS.MANAGE_WORKSPACE); // เช็คสิท
  if (!allowed) throw new AppError(403, 'Insufficient permissions');

  const workspace = await workspaceRepository.findById(workspaceId); // ดึงข้อมูล workspace โดย id
  if (!workspace) throw new AppError(404, 'Workspace not found'); 

  await deleteOldWorkspaceIcon(workspace.iconUrl ?? null); // ลบไอคอนเก่าออกจาก storage 

  const iconUrl = await uploadWorkspaceIcon(workspaceId, fileBuffer, mimeType, originalName); // อัพโหลดไอคอนใหม่ไปยัง storage และได้ public URL กลับมา
  const updated = await workspaceRepository.update(workspaceId, { iconUrl }); // อัพเดตข้อมูล workspace ใน database ด้วย iconUrl ใหม่

  return { iconUrl: updated.iconUrl }; // คืนข้อมูล iconUrl ใหม่กลับไปให้ client 
};
...
export const removeMember = async ( // ลบสมาชิกออกจาก workspace
  ...
  if (targetUserId === workspace.ownerId) { // เช็คว่า user ที่จะถูกลบออกเป็น owner ของ workspace นี้หรือไม่ 
    throw new AppError(403, 'Cannot remove the workspace owner');
  }

  if (requesterId !== targetUserId) { // ถ้าคำขอไม่เท่ากับ owner ให้ตรวจสอบสิทธิ์การลบสมาชิก
    const requester = await workspaceRepository.findWorkspaceMember(workspaceId, requesterId); // เช็คว่า user ที่ส่งคำขอลบเป็นสมาชิกของ workspace นี้หรือไม่ \
    if (!requester) { // ถ้า user ที่ส่งคำขอลบไม่ใช่สมาชิกของ workspace นี้ ให้แจ้งว่าไม่มีสิทธิ์
      throw new AppError(403, 'You are not a member of this workspace');
    }

    const allowed = await hasPermission(workspaceId, requesterId, PERMISSIONS.MANAGE_MEMBERS); 
    if (!allowed) { // ถ้า user ที่ส่งคำขอลบไม่มีสิทธิ์ในการจัดการสมาชิกของ workspace นี้ ให้แจ้งว่าไม่มีสิทธิ์
      throw new AppError(403, 'Insufficient permissions');
    }
  }

  await workspaceRepository.deleteMember(workspaceId, targetUserId); // ลบสมาชิกออกจาก workspace นี้ใน database
};
...
export const updateMemberRole = async ( // อัพเดต role ของสมาชิกใน workspace
  ...
  const workspace = await workspaceRepository.findByIdSimple(workspaceId); // เช็คว่า workspace มีอยู่จริงหรือไม่
  if (!workspace) throw new AppError(404, 'Workspace not found');

  const requester = await workspaceRepository.findWorkspaceMember(workspaceId, requesterId); // เช็คว่า user ที่ส่งคำขอเป็นสมาชิกของ workspace นี้หรือไม่
  if (!requester) {
    throw new AppError(403, 'You are not a member of this workspace');
  }

  const allowed = await hasPermission(workspaceId, requesterId, PERMISSIONS.MANAGE_MEMBERS); // เช็คว่า user ที่ส่งคำขอมีสิทธิ์ในการจัดการสมาชิกของ workspace นี้หรือไม่
  if (!allowed) {
    throw new AppError(403, 'Insufficient permissions');
  }

  if (targetUserId === workspace.ownerId) { // เช็คว่า user ที่จะถูกเปลี่ยน role เป็น owner ของ workspace นี้หรือไม่ ถ้าเป็น owner ให้แจ้งว่าไม่สามารถเปลี่ยน role ได้
    throw new AppError(403, 'Cannot change the owner role');
  }

  return workspaceRepository.updateMemberRole(workspaceId, targetUserId, data.role); // อัพเดต role ของสมาชิกใน workspace นี้ใน database และคืนข้อมูลสมาชิกที่อัพเดตแล้วกลับไป
};

export const regenerateInviteCode = async ( // สร้าง invite code ใหม่สำหรับ workspace
  workspaceId: string,
  userId: string,
) => {
  await assertWorkspaceActive(workspaceId); // เช็คว่าworkspace นี้ยัง active อยู่หรือไม่ 
  const workspace = await workspaceRepository.findByIdSimple(workspaceId); // เช็คว่า workspace มีอยู่จริงหรือไม่
  if (!workspace) throw new AppError(404, 'Workspace not found'); 

  const member = await workspaceRepository.findWorkspaceMember(workspaceId, userId); // เช็คว่า user เป็นสมาชิกของ workspace นี้หรือไม่
  if (!member) {
    throw new AppError(403, 'You are not a member of this workspace');
  }

  const allowed = await hasPermission(workspaceId, userId, PERMISSIONS.REGENERATE_INVITE); // เช็คว่า user มีสิทธิ์ในการสร้าง invite code ใหม่หรือไม่
  if (!allowed) {
    throw new AppError(403, 'Insufficient permissions');
  }

  const crypto = await import('crypto'); // นำเข้าโมดูล crypto สำหรับสร้าง UUID
  const newCode = crypto.randomUUID(); // สร้าง invite code ใหม่ด้วย UUID

  return workspaceRepository.updateInviteCode(workspaceId, newCode); // อัพเดต invite code ใหม่ใน database และคืนข้อมูล workspace ที่อัพเดตแล้วกลับไป
};
